...
class Movearmtopose(object):
    ...
    def __init__(self):
        ...
        self.yrange = (-0.5, 0.5)
        self.zrange = (0, 1)
        if(not os.path.isfile(ws_location+'index.npy')):
            np.save(ws_location+'index.npy', np.array([0]))
        self.index = np.load(ws_location+'index.npy')
        self.initial_pose = self.group.get_current_pose().pose
        self.final_pose = self.group.get_current_pose().pose
    ...
    def go_to_desired_pose(self):
        ...
        self.initial_pose = np.array(self.group.get_current_pose().pose)
        ...
        current_pose = self.group.get_current_pose().pose
        self.final_pose = np.array(self.group.get_current_pose().pose)
    ...
    def get_plan(self):
        if(len(self.positions)):
            dic = {'plan':self.positions, 'Initial Pose': self.initial_pose, 'Final pose': self.final_pose}
            dbfile = open(ws_location+'plan'+str(self.index[0])+'.pickle', 'ab+')
            pickle.dump(dic, dbfile)
            dbfile.close()
            # np.save('plan_'+str(self.index)+'.npy', self.positions)
            # print("Got plan {0}".format(self.index))
            self.index+=1
            np.save(ws_location+'index.npy', np.array(self.index))
...
>>>>>>> 5baef12ca4d9ab3625f041be27b7aa84a11efdf9
        np.save(ws_location+'pcloud_1.npy', np.array(lis))
