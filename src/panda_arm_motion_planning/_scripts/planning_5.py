...
class Movearmtopose(object):
    ...
    def go_to_desired_pose(self):
        ...
        for i in range(len(plan.joint_trajectory.points)):
            for j in range(len(plan.joint_trajectory.points[0].positions)):
                sheet.cell(row=i + 2 + start_col_ind, column=j + 1).value = plan.joint_trajectory.points[i].positions[j]
        wb.save('../dataset/testfile.xlsx')

        # start_col_ind +=len(plan)
    ...
    def get_obstacle_data(self):
        self.ob_sub = rospy.Subscriber('/collision_object', CollisionObject, self.print_function)
